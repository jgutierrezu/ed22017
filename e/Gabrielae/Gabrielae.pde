...
Lo más general es la visualización de daots de la temperatura, para luego 
enforcarnos en la humedad por cada mes (maxor humedad, media humedad y menor humedad),
hay meses en los cuales no se manifiestancambien tan brusco y por esto no 
...
void setup() {
  ...
  for (int i = 0; i < tabla.getRowCount(); i++) {
    ...
    humedad[i] = new Meses(mes, in, minH, meanH, maxH, nubes, prec);
  }
}
